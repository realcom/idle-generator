...
| `Items.json` | 74 | 재료, 티켓, 옵션/레벨 포인트, 무기 15종, 장비 30종, 스킬북, 스킬 학습 레시피, shop 상품, 출석판 추가 |
| `Skills.json` | 4 | 단일딜, 광역, 보스딜, 자기 버프 스킬 구성 |
| `Buffs.json` | 1 | 공격/공속 성장형 자기 버프 추가 |
| `Maps.json` | 15 | 메인 5스테이지, 일반 던전 3종, 요일 던전 7종 |
| `Achievements.json` | 26 | 튜토리얼/예약 업적 외 성장 유도 업적 6종과 레벨 포인트 자동 지급 업적 추가 |
| `Audios.json` | 1 | 기존 전투 BGM 유지 |
| `Triggers.json` | 11 | 기존 `mushroom_field` 전투 트리거 재사용 |
...
MVP 스킬 트리는 엔진의 기존 Item/Recipe/LevelUpItem 경로 안에서 표현한다.

| 요소 | 하네스 표현 | 현재 데이터 |
| --- | --- | --- |
| 트리 노드 | `Item.Category=Skill`, `type=PlayerUnitActiveSkill` | `200401`~`200404` |
| 스킬 연결 | `ResourceSkill.itemDataId` + `SkillItem.skillDataId` | `300101`~`300104` ↔ `200401`~`200404` |
| 기본 지급 | 루트 스킬 아이템 `initialCreate=true` | `포자 충격서` |
| 학습 비용 | `Item.Category=Recipe.materialItemGroups` | `200405`~`200407` |
| 레벨업 비용 | `SkillItem.levelUpMaterialItemGroups[level]` | Lv1→Lv5 비용 |
| 선행 조건 | `requiredItemDataIds`, `targetItemDataIds`, `popupArgs` | 루트 → 광역/보스 → 각성 |
| 소모 포인트 | 신규 `레벨 포인트` material | `200107` |
| 포인트 자동 지급 | 반복 업적 `LevelUpItem(playerLevel=1)` | `600107` |

학습은 `CreateItem(recipeItemDataId)`로 레시피를 실행해 스킬 아이템을 획득하는 방식으로
표현한다. 스킬 레벨업은 기존 `LevelUpItem` 경로가 각 스킬 아이템의
`levelUpMaterialItemGroups[level]`을 소비하도록 구성했다.

플레이어 레벨업 시점의 `레벨 포인트` 지급은 엔진 훅을 새로 만들지 않고 업적 경로를
정본으로 사용한다. `600107 레벨 포인트 지급`은 `condition=LevelUpItem`,
`conditionValue1=1`, `targetProgress=1`, `repeatable=true`, `autoReward=true`로 구성되어
플레이어 레벨 아이템이 1회 레벨업할 때마다 `200107 레벨 포인트` 1개를 자동 지급한다.
...
- 인게임 재화 구매 비용은 `productMaterialItemGroups`만 사용한다.
- `priceXtr`은 legacy 결제 필드로 간주하고 신규 `mushroomer` 콘텐츠에는 사용하지 않는다.
- 실결제 상품이 필요해질 때만 `priceUsd`/`priceWon`/`iapIdentifier` 경로를 별도 검증한다.
...
| 레벨 포인트 | 신규 Material `id=200107` | 스킬 트리 학습/레벨업 |
...
| 스킬 | `ResourceSkill.itemDataId` + `Item.Category=Skill` + 학습 `Recipe` |
...
| 스킬 | `skills/*.skill.yaml`, `skills/*.behavior.yaml`, skill item, recipe | 공격/광역/보스딜/버프 스킬이 스킬 트리 노드와 레벨 포인트 비용에 연결됨 |
...
| 스킬 | 가능 | `ResourceSkill` timeline/trigger와 `Item.Category=Skill` 사용 |
| 스킬 트리 데이터 | 가능 | `parentId`, `targetItemDataIds`, `requiredItemDataIds`, `popupArgs`로 노드/선행조건 표현 |
| 스킬 학습/레벨업 비용 | 가능 | 학습은 `Recipe`, 레벨업은 `LevelUpItem`의 `levelUpMaterialItemGroups[level]` 사용 |
| 레벨업 시 포인트 자동 지급 | 가능 | 반복 업적 `600107`이 `LevelUpItem(playerLevel=1)` 이벤트를 받아 자동 보상 지급 |
...
- 스킬 트리 루트, 학습 레시피, 레벨업 비용이 컴파일 산출물에 보존된다.
- 플레이어 레벨업 1회당 `레벨 포인트` 1개가 반복 업적으로 자동 지급된다.
...
- 스킬 트리 UI가 `Recipe`/`Skill` 아이템 메타데이터를 읽어 학습/레벨업 버튼으로 이어질 수 있는지
