...
- 액션 인자는 문서에 있는 것만 쓴다. 예전 샘플의 `target: caller`처럼 컴파일러가 쓰지 않는 인자는 금지된다.
- `AddUnit`에는 반드시 정의된 `unitDataId`를 넣고, `locationId`를 쓰면 대상 맵의 `locations[].geometries[]`를 확인한다.
- damage/heal trigger에서 대형 값을 관찰만 할 때는 `Return`을 설정하지 않는다.
