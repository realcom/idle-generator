...
- 액션 인자는 문서에 있는 것만 사용. 컴파일러가 모르는 인자는 error.
- `AddUnit`은 `unitDataId`/`count`/`locationId`를 검증한다. `locationId`를 쓰면 맵 location과 geometry를 함께 만든다.
- 대형 damage/heal을 trigger에서 수정하지 않으면 `Return`을 쓰지 않는다.
