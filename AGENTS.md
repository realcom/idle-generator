...
> 이 파일은 Codex가 이 모노리포에서 작업할 때 빠르게 참조하는 단축 정보.
...
| `.Codex/agents/` | 작성 에이전트 정의 | ✅ |
| `.Codex/skills/` | 작성 스킬 정의 (gen-*, new-content 등) | ✅ |
...
1. **콘텐츠/AI 발산** (harness + .Codex) — RPG maker로서 발산력·재사용성
2. **end-to-end 라스트마일** — harness 산출물이 실제 engine에 로드되는 경로 확립
3. 엔진 자체 확장은 **최후 수단** (정말 필요할 때만)

자세한 방향성: `~/.Codex/projects/-Users-yangjinhwan-Documents-Codex-Projects-idle-game-generator/memory/` (project_direction.md, commons-policy.md, asset-generator-integration.md)
