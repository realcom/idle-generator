    ...
    SubShader
    {
        ...
        CGPROGRAM
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                const float deg = atan2(IN.texcoord.y - edgeShinyCenter.y, IN.texcoord.x - edgeShinyCenter.x) / (UNITY_PI*offset);
                bool isShine =  frac(_Time.y * edgeShinyAutoPlaySpeed  + deg) < edgeShinyWidth;

                fixed4 finalEdgeColor = lerp(edgeColor, edgeShinyColor, isShine);

                color.rgb = lerp(finalEdgeColor, color, color.a >= 0.9);
                color.a *= lerp(finalEdgeColor.a, 1, color.a >= 0.9);
                color.a = color.a > 0.1 ? 1 : 0;
