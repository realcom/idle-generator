    ...
    SubShader
    {
        ...
        CGPROGRAM
            ...
            UNITY_INSTANCING_BUFFER_START(Props)
                ...
                UNITY_DEFINE_INSTANCED_PROP(float, _FillOffset)
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                float fillOffset = UNITY_ACCESS_INSTANCED_PROP(float, _FillOffset);

                //! FOR TEST
                //alphaOffset = frac(_Time.x*10);

                
                edgeShinyCount = edgeShinyCount <= 0 ? 1 : edgeShinyCount;
                float offset = 1 / edgeShinyCount;
                const float deg = atan2(IN.texcoord.y - 0.5, IN.texcoord.x - 0.5) / (UNITY_PI*2);
                ...
                float finalSmoothness = 0.1f;

                float fracOffset = (2 * fillOffset) - 1;
                fracValue += fracOffset; // (-1~0) -> (1, 2)
                fracValue = saturate(fracValue);
                fracValue = fracValue <= fillOffset ?
                    Unity_Remap_float(fracValue, float2(0, fillOffset), float2(0,1))
                    : Unity_Remap_float(fracValue, float2(fillOffset, 1 + fracOffset), float2(1, 0));


                color.a *= fracValue ;
                color.a = min(saturate(color.a), defaultAlpha);
                color *= IN.color;
