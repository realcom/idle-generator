...
//from https://forum.unity.com/threads/text-mesh-pro-does-not-work-with-spriteatlas-assets.697088/
[CreateAssetMenu]
public class TexturePackerAsset : ScriptableObject
{
    public bool overwriteExisting = true;

    [ValueDropdown("maxSizeOptions")] public int maxSize = 512;

    [FolderPath(ParentFolder = "Assets/", RequireExistingPath = true)]
    public string outputFolder;

    [Range(0, 10)] public int padding = 2;
    ...
    public void RepackAtlas()
    {
        ...
        //Save image
        var path = "Assets/" + outputFolder + "/" + name;
    ...
    private List<int> maxSizeOptions = new List<int> { 256, 512, 1024, 2048, 4096, 8192, 16384, 32768 };
    ...
    private Texture2D[] GatherTextures()
    {
        ...
        var inFolders = Enumerable.Empty<Texture2D>();
        if (folderPaths.Length > 0 && !string.IsNullOrEmpty(folderPaths[0]))
        {
            inFolders = AssetDatabase
                .FindAssets(searchQuery, folderPaths)
                .Select(AssetDatabase.GUIDToAssetPath)
                .Select(AssetDatabase.LoadAssetAtPath<Texture2D>);
        } 
        
        return inFolders.Concat(textures).ToArray();
    }
