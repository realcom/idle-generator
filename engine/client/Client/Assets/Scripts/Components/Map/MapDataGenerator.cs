...
public class MapDataGenerator : ZMonoBehaviour
{
 ...
 //    {
	...
	//     if (mapData == null)
	//     {
	// 	    var fileFullPath = $"Assets/PatchResources/Maps/MapData/{sceneName}.asset";
	// 	    mapData = ScriptableObject.CreateInstance<MapData>();
	// 	    AssetDatabase.CreateAsset(mapData, fileFullPath);
 ...
 //    {
	...
	// 	    InitMapData();
	//     
	//     mapData.CalculatePostData();
	//     ExportCurrentSceneTerrainLocation(Path.GetFileNameWithoutExtension(EditorSceneManager.GetActiveScene().path));
	//     
	//     mapScreen.Initialize(mapData);
 //    }
 //
 ...
 //
	//     // 사각형 꼭지점 좌표 설정
	//     vertices[0] = new Vector3(mapData.boundsMin.x, mapData.boundsMin.y);
	//     vertices[1] = new Vector3(mapData.boundsMin.x, mapData.boundsMax.y);
	//     vertices[2] = new Vector3(mapData.boundsMax.x, mapData.boundsMax.y);
	//     vertices[3] = new Vector3(mapData.boundsMax.x, mapData.boundsMin.y);
 //
	//     // 삼각형 인덱스 설정
	//     triangles[0] = 0;
	//     triangles[1] = 1;
 ...
 //    {
	...
	// 	    Gizmos.DrawWireCube(mapData.boundsOffset.X0Z(), mapData.boundsSize.X0Z());
	//     }
 //    }
}
