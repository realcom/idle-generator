...
public class StaticBlurCapture : RawImage
#if UNITY_EDITOR
...
#endif
{
    ...
    [SerializeField] BlurMode m_BlurMode = BlurMode.Blur;
    ...
    IEnumerator _CoUpdateTextureOnNextFrame()
    {
        ...
        Texture fullScreenTexture = Shader.GetGlobalTexture("_FullGrapPass");
        RenderTexture.active = _rt;
        //Graphics.Blit(fullScreenTexture, _rt);
        ...
        if (_buffer == null)
		{
            ...
            _buffer.Blit(fullScreenTexture, s_CopyId);
				...
				else
			{
                ...
                var rt_effect1 = new RenderTexture(w, h, 0);
                rt_effect1.filterMode = m_FilterMode;
                rt_effect1.wrapMode = TextureWrapMode.Clamp;

                var rt_effect2 = new RenderTexture(w, h, 0);
                rt_effect2.filterMode = m_FilterMode;
                rt_effect1.wrapMode = TextureWrapMode.Clamp;

                Graphics.Blit(fullScreenTexture, rt_effect1, mat);


                mat.SetFloat("_offset", 1.5f * m_Blur);
                // Iterate the operation.
                if (1 < m_Iterations)
				{
                    for (int i = 1; i < m_Iterations; i++)
                    {
                        mat.SetFloat("_offset", (0.5f + i) * m_Blur);
                        // Apply effect (effect1 -> effect2, or effect2 -> effect1).
                        Graphics.Blit(i % 2 == 0 ? rt_effect2 : rt_effect1, i % 2 == 0 ? rt_effect1 : rt_effect2, mat);
                    }
				}

                Graphics.Blit(m_Iterations % 2 == 0 ? rt_effect2 : rt_effect1, _rt);
                //rt_effect1.Release();
                rt_effect1 = null;
                if (1 < m_Iterations)
				{
        ...
        copiedTexture = new Texture2D(_rt.width, _rt.height, TextureFormat.ARGB32, false, true);
        copiedTexture.filterMode = m_FilterMode;
        copiedTexture.wrapMode = TextureWrapMode.Clamp;
        copiedTexture.hideFlags = HideFlags.HideAndDontSave;

        copiedTexture.ReadPixels(new Rect(0, 0, _rt.width, _rt.height), 0, 0);
        copiedTexture.Apply();
