...
const TABS := ["sanctuary", "equipment", "exploration", "missions", "shop"]
...
func _capture() -> void:
	...
	for tab in TABS:
		home._open_tab_modal(str(tab))
		await process_frame
		await process_frame
		_save_viewport(root, "res://screenshots/godot-ninja2-tab-%s.png" % str(tab))
		if home.modal_layer != null:
			home.modal_layer.close_all()
		await process_frame

	print("godot-ninja2 tab modal captures written")
	quit(0)
