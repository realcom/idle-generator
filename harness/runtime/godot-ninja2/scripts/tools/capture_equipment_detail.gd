...
const OUTPUT_PATH := "res://screenshots/godot-ninja2-equipment-detail.png"
...
func _capture() -> void:
	...
	if not content_ok or not housing_ok:
		_fail("equipment detail capture data load failed")
		return
	...
	home._open_tab_modal("equipment")
	await process_frame
	await process_frame
	home._open_equipment_detail_modal(int(sanctuary_state.selected_equipment_item_id))
	await process_frame
	await process_frame
	_save_viewport(root, OUTPUT_PATH)
	print("godot-ninja2 equipment detail capture: %s" % ProjectSettings.globalize_path(OUTPUT_PATH))
	quit(0)
