...
func build_plan(map_def: Dictionary) -> Array:
	...
	var start_triggers := _ordered_start_triggers(trigger_names)
	var board_vars := _board_vars_from_map(map_def)
	var waves := []

	for trigger_name in start_triggers:
		...
		waves.append({
			...
			"is_boss": str(trigger_name).contains("BOSS"),
		})

	return waves


func _ordered_start_triggers(trigger_names: Array) -> Array:
	var result := []
	var update_triggers := []
	...
	result.sort_custom(func(a, b): return _wave_sort_key(str(a)) < _wave_sort_key(str(b)))
	return result


func _board_vars_from_map(map_def: Dictionary) -> Dictionary:
	var board_vars := {
		601: 1.0,
		603: 0.0,
		604: 0.0,
	...
	for variable in map_def.get("initVariables", []):
		if typeof(variable) != TYPE_DICTIONARY:
			continue
		board_vars[int(variable.get("callerKey", 0))] = float(variable.get("value", 0.0))
	return board_vars


func _wave_sort_key(trigger_name: String) -> int:
	if trigger_name.contains("BOSS"):
		return 999
	var regex := RegEx.new()
	regex.compile("WAVE(\\d+)")
	var hit := regex.search(trigger_name)
	return int(hit.get_string(1)) if hit != null else 0
