...
| **디자인 리소스** | UI 시안·아트디렉션·레이아웃 토큰·component-blueprints·컴포넌트 스킨·asset-plan·Phaser spec·Unity UI recipe | `design/<game>/`, `runtime/specs/ui/`, `unity/recipes/` | Phaser runtime UI / uGUI prefab / 검증 스크린샷 |

> 핵심 계약: **데이터는 에셋을 "키(경로/이름)"로만 참조한다.** (예: `sprite: "Units/Slime/slime_green.png"`)
> 이 분리 덕에 데이터와 에셋을 독립적으로 만들고, 검증기가 "키 ↔ 실제 에셋" 매칭을 확인한다.
> 디자인은 바로 런타임이나 prefab을 쓰지 않고 **시안 → component-blueprints/토큰/스킨/asset-plan → 플랫폼별 spec/recipe → Phaser runtime 또는 Unity prefab 생성** 순서로 흐른다.
> `component-blueprints.yaml`은 의미 컴포넌트, 박스 모델, `slice_hints`/`content_insets`, ornament layer, runtime token mapping의 기준 파일이다.
> 디자인 런타임은 섹션별로 **시안 crop → 런타임 crop → 비교판 → 판정(PASS/WARN/ERROR) → 다음 수정 범위**를 남기며 이터레이션한다.
...
② 생성(Generate)  ├─ 데이터: AI가 schema+profile 지켜 content YAML/growth/behavior 생성
        ...
        │         └─ 디자인: 시안 생성 → component-blueprints/art-direction/layout/component-skin/asset-plan 추출 → genimg 에셋 생성
        │
...
④ 검증(Validate)  tools/validate: 스키마·참조무결성·스탯 가드레일·성장식·행동어휘
        ...
        │         + 디자인 리뷰(모바일성·전투 가시성·구현 가능성) + 시안/런타임 비교판 + component blueprint 검증 + Phaser smoke + Unity UI recipe 검증
        │
⑤ 컴파일(Compile) 소스 → 엔진 proto-JSON (성장식→배열, behavior→Triggers.json)
        │         에셋 → Addressables 번들
        │         Phaser spec → runtime UI, Unity recipe → uGUI prefab / scene skin
        │
...
| ② 생성(디자인) | `gen-ui-concept` / `extract-design-system` / `gen-ui-assets` | UI 시안 생성, component-blueprints·디자인 토큰·컴포넌트 스킨·asset-plan 추출, genimg 에셋 생성 |
| ③~⑤ Phaser UI | `gen-phaser-ui-spec` / `build-phaser-ui-runtime` | Phaser/HTML 구현 명세 작성, 런타임 반영, smoke QA |
| ③~⑤ Unity UI | `gen-unity-ui-recipe` / `build-unity-ui-prefab` | Unity recipe 작성, uGUI prefab 생성 |
| ④ 검증/⑦ 리뷰 | `content-reviewer` | validate 실행 + 일관성·참조무결성·밸런스 리뷰 |
| ④ 디자인 리뷰 | `design-review` | 모바일성·전투 가시성·캐릭터 정체성·구현 가능성 리뷰 + 시안/런타임 비교판 판정 |
...
| `/extract-design-system` | 선택된 시안에서 디자인 토큰·스킨·asset-plan 추출 |
...
| `/gen-unity-ui-recipe` | 인게임 HUD/하단바 Unity recipe 작성 |
...
- **UI 아이콘은 imagegen 원본 필수**: 플레이어가 보는 최종 UI 아이콘은 built-in `imagegen`/`image_gen`으로 생성하고, 스크립트는 alpha/crop/resize/packing 후처리에만 쓴다. 원본과 후처리 경로는 `asset-plan.yaml`에 남긴다.
