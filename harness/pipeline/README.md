...
| **디자인 리소스** | UI 시안·아트디렉션·레이아웃 토큰·컴포넌트 스킨·asset-plan·Phaser spec·Unity UI recipe | `design/<game>/`, `runtime/specs/ui/`, `unity/recipes/` | Phaser runtime UI / uGUI prefab / 검증 스크린샷 |

> 핵심 계약: **데이터는 에셋을 "키(경로/이름)"로만 참조한다.** (예: `sprite: "Units/Slime/slime_green.png"`)
> 이 분리 덕에 데이터와 에셋을 독립적으로 만들고, 검증기가 "키 ↔ 실제 에셋" 매칭을 확인한다.
> 디자인은 바로 런타임이나 prefab을 쓰지 않고 **시안 → 토큰/스킨/asset-plan → 플랫폼별 spec/recipe → Phaser runtime 또는 Unity prefab 생성** 순서로 흐른다.
...
② 생성(Generate)  ├─ 데이터: AI가 schema+profile 지켜 content YAML/growth/behavior 생성
        ...
        │         └─ 디자인: 시안 생성 → art-direction/layout/component-skin/asset-plan 추출 → genimg 에셋 생성
        │
...
④ 검증(Validate)  tools/validate: 스키마·참조무결성·스탯 가드레일·성장식·행동어휘
        ...
        │         + 디자인 리뷰(모바일성·전투 가시성·구현 가능성) + Phaser smoke + Unity UI recipe 검증
        │
...
| ② 생성(디자인) | `gen-ui-concept` / `extract-design-system` / `gen-ui-assets` | UI 시안 생성, 디자인 토큰·컴포넌트 스킨·asset-plan 추출, genimg 에셋 생성 |
